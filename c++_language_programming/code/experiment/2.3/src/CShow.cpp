...
void CShow::Menu() {
	while (true) {
		...
		cout<<"              4、根据车牌号查询交通工具"<<endl;
		cout<<"              5、根据所有人查询交通工具"<<endl;
		cout<<"                       6、统计"<<endl;
		cout<<"                       7、退出"<<endl;
		cout<<"请输入选项前的标号："; 
		cin>>choose;
		...
		switch(choose[0]) {
			...
			case '3':system("cls");PManager.putPersonList();break;
			case '4':system("cls");SearchVehicleWithNo();break;
			case '5':system("cls");SearchVehicleWithOwner();break;
			case '6':system("cls");ReportCount();break;
...
void CShow::addVehicle() {
	...
	if (pv!=nullptr) {
		...
		cout<<"VID\tBrand\tColor\tVIN\tOName\tDName\tText"<<endl;
		pv->putAll();
		po = pv->getOwner();
		pd = pv->getDriver();
		if (po!=nullptr) {
			cout<<"ID\tName\tAge\tGender\tDLTime\tText"<<endl;
			pv->getOwner()->putAll();
			if (pd!=nullptr) {
				cout<<"ID\tName\tAge\tGender\tDLTime\tText"<<endl;
				pv->getDriver()->putAll();
